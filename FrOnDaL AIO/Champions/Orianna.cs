    ...
    public class Orianna
    {
        ...
        private static void SpellDraw(EventArgs args)
        {
            ...
            if (Main[Gamer.CharacterName]["drawings"]["cleardraw"] && Gamer.IsVisibleOnScreen)
            {
                ...
                {
                    RenderDrawText("LaneClear: On", Gamer.HPBarPosition + new Vector2(-35, 160), Color.LawnGreen);
                }
                else
                {
                    RenderDrawText("LaneClear: Off", Gamer.HPBarPosition + new Vector2(-35, 160), Color.Red);
                }
            }
            if (Main[Gamer.CharacterName]["drawings"]["harasdraw"] && Gamer.IsVisibleOnScreen)
            {
                ...
                {
                    RenderDrawText("Auto harass: On", Gamer.HPBarPosition + new Vector2(-35, 180), Color.LawnGreen);
                }
                else
                {
                    RenderDrawText("Auto harass: Off", Gamer.HPBarPosition + new Vector2(-35, 180), Color.Red);
                }
            }
        }
       ...
       /* private static void AutoShield(AIBaseClient sender, AIBaseClientProcessSpellCastEventArgs args)
                ...
                if (Q.IsReady() && Main[Gamer.CharacterName]["laneClear"]["clearQ"])
                {
                    ...
                    if (qPredMinion.MinionsHit == 0)
                    {
                        foreach (var target in GameObjects.EnemyMinions.Where(x => x.IsValidTarget(Q.Range)))
                        {
                            if (target != null)
                            {
                                if (GameObjects.EnemyMinions.Count(x => x.IsValidTarget(170, true,Q.GetPrediction(target).CastPosition)) >= Main[Gamer.CharacterName]["laneClear"]["clearCountQ"].GetValue<MenuSlider>().Value)
                                {
        ...
        private static void AntiGapCloser(AIHeroClient sender, Gapcloser.GapcloserArgs args)
        {
            ...
            {
                if (E.IsReady() && Gamer.Position.Extend(Game.CursorPos, E.Range).CountEnemyHeroesInRange(400) < 3)
                {
                    if (sender.IsValidTarget(E.Range))
                    {
                        E.Cast(Gamer.Position.Extend(Game.CursorPos, E.Range), true);
                    }
                }
            }
        }
