...
namespace FrOnDaL_AIO.Common.Utilities.SpellBlocking
{
    public static class EvadeManager
    {
        ...
        {
            Menu = new Menu("EvadeSpells", "EvadeSpells");

            Collision.Init();

            EvadeSpellMenu = new Menu("evadeSpells", "EvadeSpells");
            {
                foreach (var spell in EvadeSpellDatabase.Spells)
                {
                    var name = spell.Slot;
                    EvadeSpellMenu.Add(new MenuSeparator("FioraEvade_" + name, ObjectManager.Player.CharacterName + " " + name + " Settings"));
                    EvadeSpellMenu.Add(new MenuSlider("DangerLevel_" + name, "-", spell.DangerLevel, 1, 5));
                    EvadeSpellMenu.Add(new MenuBool("Enabled" + name, "+"));
                }
            }
                      ...
                      SpellDatabase.Spells.Where(
                          i => GameObjects.EnemyHeroes.Any(a => a.CharacterName == i.ChampionName)))
                        {
                            ...
                            if (String.Equals(spell.ChampionName, hero.CharacterName, StringComparison.InvariantCultureIgnoreCase))
                            {
                                ...
                                heroMenu.Add(new MenuSlider("DangerLevel" + spell.MenuItemName, "-", spell.DangerValue, 1, 5));
                                heroMenu.Add(new MenuBool("Enabled" + spell.MenuItemName, "-"));
                ...
                ObjectManager.Player.CharacterName == "Karma")
            {
                bestAllies = GameObjects.AllyHeroes
                    .Where(t =>
                        t.Distance(ObjectManager.Player) < spellMisc.E.Range)
                    .OrderBy(x => x.Health);
            }
...

            {
                bestAllies = GameObjects.AllyHeroes
                    .Where(t =>
                        t.Distance(ObjectManager.Player) < spellMisc.W.Range)
                    .OrderBy(x => x.Health);
            }
        ...
        private static void OnUpdate(EventArgs args)
        {
            ...
            if (bestAllies != null)
            {
                foreach (var ally in bestAllies)
                {
                    ...
                    if (!IsSafe(ally.Position.ToVector2()).IsSafe)
                    {
                        TryToEvade(IsSafe(ally.Position.ToVector2()).SkillshotList, Game.CursorPos.ToVector2());
                    }
                }
            }
        }
...

                        {
                            ...
                            if (evadeSpell.Slot == SpellSlot.E)
                            {
                                if (spellList.Any())
                                {
                                    ...
                                    if (willHitList.Any())
                                    {
                                        if (SpellBlocking.EvadeTargetManager.Menu["whitelist"][
