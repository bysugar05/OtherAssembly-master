        ...
        public static float GetDamageByChampion(AIBaseClient target)
        {
            ...
            switch (OlympusAIO.objPlayer.CharacterName)
            {
                ...
                case "Evelynn":
                    ...
                    if (SpellManager.Q.IsReady())
                    {
                        damage += SpellManager.Q.GetDamage(target, Evelynn.Misc.IsWEmpowered() ? DamageStage.Empowered : DamageStage.Default);
                    }
                    ...
                    if (SpellManager.R.IsReady())
                    {
                        damage += SpellManager.R.GetDamage(target, target.HealthPercent < 30 ? DamageStage.Empowered : DamageStage.Default);
                    }
                    break;
                case "Heimerdinger":
                    if (OlympusAIO.objPlayer.CanAttack)
