...
namespace OlympusAIO.Champions
{
    class Lissandra
    {
        ...
        public static void OnLoad()
        {
            ...
            SpellManager.Q.SetSkillshot(0.5f, 75f, 2200f, false, SkillshotType.Line);
            SpellManager.E.SetSkillshot(05f, 125f, 850f, false, SkillshotType.Line);

            /* Main */
            Game.OnUpdate += Events.OnUpdate;

            /* GameObject */
            GameObject.OnMissileCreate += Events.OnMissileCreate;
        ...
        public class Events
        {
            ...
            public static void OnGapcloser(AIHeroClient sender, Gapcloser.GapcloserArgs args)
            {
                ...
                {
                    if (args.Type == Gapcloser.SpellType.Targeted && objPlayer.HealthPercent <= 20)
                    {
                        SpellManager.R.CastOnUnit(objPlayer);
                    }
                    else if (args.Type == Gapcloser.SpellType.SkillShot && args.EndPosition.DistanceToPlayer() <= 200)
                    {
                        SpellManager.R.CastOnUnit(objPlayer);
                    }
                }
