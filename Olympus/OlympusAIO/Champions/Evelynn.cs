    ...
    class Evelynn
    {
        ...
        public class Events
        {
            ...
            public static void OnGapcloser(AIHeroClient sender, Gapcloser.GapcloserArgs args)
            {
                ...
                if (MenuManager.MiscGapcloserMenu["R"].GetValue<MenuBool>().Enabled && SpellManager.R.IsReady() && args.Type == Gapcloser.SpellType.SkillShot)
                {
        ...
        public class Modes
        {
            public static void Combo()
            {
                if (MenuManager.ComboMenu["W"].GetValue<MenuBool>().Enabled && SpellManager.W.IsReady())
                {
                    ...
                    var enemies = TargetSelector.GetTargets(SpellManager.W.Range);
                    ...
                    switch (MenuManager.ComboMenu["WSelectBy"].GetValue<MenuList>().SelectedValue)
                    {
                        ...
                        case "Lowest Health":
                            ...
                            break;
                        case "Most Priority":
                            target = enemies.MinBy(x => TargetSelector.GetPriority(x) == TargetPriority.Max);
                            break;
                    }
                    ...
                    if (!Misc.IsAllured(target) && target.IsValidTarget(SpellManager.W.Range) && MenuManager.ComboWhiteList[target.CharacterName.ToLower()].GetValue<MenuBool>().Enabled)
                    {
                ...
                if (MenuManager.ComboMenu["R"].GetValue<MenuBool>().Enabled && SpellManager.R.IsReady())
                {
                    ...
                    {
                        if (MenuManager.ComboRWhiteList[target.CharacterName.ToLower()].GetValue<MenuBool>().Enabled && SpellManager.R.GetDamage(target, target.HealthPercent < 30 ? DamageStage.Empowered : DamageStage.Default) >= UtilityManager.GetTargetHealthWithShield(target))
                        {
        ...
        public class Misc
        {
            public static void KillSteal()
            {
                ...
                {
                    foreach (var target in  GameObjects.EnemyHeroes.Where(x => x.IsValidTarget(SpellManager.E.Range) && SpellManager.E.GetDamage(x, Misc.IsWEmpowered() ? DamageStage.Empowered : DamageStage.Default) >= UtilityManager.GetTargetHealthWithShield(x)))
                    {
                        SpellManager.E.CastOnUnit(target);
                        break;
                    }
                ...
                {
                    foreach (var target in GameObjects.EnemyHeroes.Where(x => x.IsValidTarget(SpellManager.R.Range) && SpellManager.R.GetDamage(x, x.HealthPercent < 30 ? DamageStage.Empowered : DamageStage.Default) >= UtilityManager.GetTargetHealthWithShield(x)))
                    {
            ...
            public static void SemiManualR()
            {
                ...
                foreach (var target in GameObjects.EnemyHeroes.Where(x => x.IsValidTarget(SpellManager.R.Range) && x.Health <= SpellManager.R.GetDamage(x, x.HealthPercent < 30 ? DamageStage.Empowered : DamageStage.Default)))
                {
